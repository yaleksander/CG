...
// Materiais para indicar quando o teleporte esta disponivel
var ringMatYes = new THREE.MeshBasicMaterial({
	color: 0x00ff00,
	opacity: 0.8,
	transparent: true
});
var ringMatNo = new THREE.MeshBasicMaterial({
	color: 0xff0000,
	opacity: 0.8,
	transparent: true
});
...
function onSelectStart(event)
{
	...
	if (intersections.length > 0 && intersections[0].object.name == "pass")
		cameraHolder.position.set(intersections[0].point.x, defaultHeight, intersections[0].point.z);
}
...
function intersectObjects(controller)
{
	...
	if (intersections.length > 0 && intersections[0].object.name == "pass")
		rectile.material = ringMatYes;
	else
		rectile.material = ringMatNo;
}
